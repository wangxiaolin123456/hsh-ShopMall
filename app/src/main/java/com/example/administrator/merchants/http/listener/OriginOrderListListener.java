...
public class OriginOrderListListener implements Response.Listener<JSONObject> {
    ...
    public void onResponse(JSONObject jsonObject) {
        LogUtil.i("原产地订单"+jsonObject.toString());
        if (Status.status(jsonObject)) {
            ...
            try {
                ...
                for (int j = 0; j < jsonObjectList.size(); j++) {
                    ...
                    originOrderShowBean.setIsSilver(jsonObjectList.get(j).getString("isSilver"));;//是否使用银贝币
                    if ("1".equals(jsonObjectList.get(j).getString("ordstatus"))
                            && "1".equals(jsonObjectList.get(j).getString("paystatus"))) {
                        ...
                        originOrderShowBean.setOrderMoney(jsonObjectList.get(j).getString("payamt"));
                    } else if ("0".equals(jsonObjectList.get(j).getString("ordstatus"))
                            && "1".equals(jsonObjectList.get(j).getString("paystatus"))) {
