 ...
 * 功能：贝币发放记录
 */
public class BeiBiGrantRecordListener implements Response.Listener<JSONObject> {
    ...
    public BeiBiGrantRecordListener(Context context, int type, List<BeiRecordShowBean>list,
                                    ListView listView, RefreshLayout swipeLayout,
        ...
        this.beiBiGrantRecordAdapter=beiBiGrantRecordAdapter;
        this.footView=footView;
    }
    @Override
    public void onResponse(JSONObject jsonObject) {
        LogUtil.i("贝币发放记录"+jsonObject.toString());
        if (Status.status(jsonObject)){
            //返回成功
            JSONArray jsonArray = null;
            try {
                jsonArray = jsonObject.getJSONArray("inList");
                ...
                for (int j = 0; j < jsonObjectList.size(); j++) {
                    ...
                    beiRecordShowBean.setInoutobjdescr(jsonObjectList.get(j).getString("ordno"));//
...
//                    }
                ...
                if (type==0){
                    ...
                    listView.setAdapter(beiBiGrantRecordAdapter);
                }else if (type==1){
                    //加载
                    BeiBiRecordActivity.s=1;
                    swipeLayout.setLoading(false);
                    beiBiGrantRecordAdapter.notifyDataSetChanged();
                }
